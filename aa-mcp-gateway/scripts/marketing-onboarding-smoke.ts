/** Gate 9 uses the existing Harbour bridge, without changing any connector. */
import assert from "node:assert/strict";
import { readFileSync, statSync } from "node:fs";
import { Client } from "@modelcontextprotocol/sdk/client/index.js";
import { StdioClientTransport } from "@modelcontextprotocol/sdk/client/stdio.js";
import { marketingDirector as config } from "../src/onboarding/marketing-director.js";
import { assertMarketingDiscovery, runMarketingGate } from "./marketing-gate.js";
const client = new Client({ name: "aa-marketing-gate9", version: "1.0.0" });
let calls = 0;
try {
  ...
  assertMarketingDiscovery(names);
  await runMarketingGate(async (name, args) => {
    calls++;
