...
import { assertFinanceDiscovery, runFinanceGate } from "./finance-gate.js";
const client = new Client({ name: "aa-finance-gate13", version: "1.0.0" });
let calls = 0;
try {
  ...
  const bad = await fetch(url, {
    ...
    headers: {
      authorization: "Bearer gate13-invalid-test-credential",
  ...
  await client.connect(
    new StdioClientTransport({
      command: financeController.connector.command,
  ...
  assertFinanceDiscovery(names);
  await runFinanceGate(async (name, args) => {
    calls++;
