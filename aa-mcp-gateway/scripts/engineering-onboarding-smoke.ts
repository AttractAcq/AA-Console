...
const client = new Client({ name: "aa-engineering-gate14", version: "1.0.0" });
let calls = 0;
try {
  ...
  const bad = await fetch(url, {
    ...
    headers: {
      authorization: "Bearer gate14-invalid-test-credential",
  ...
  await client.connect(
    new StdioClientTransport({
      command: engineeringOps.connector.command,
  ...
  assertEngineeringDiscovery(names);
  await runEngineeringGate(async (name, args) => {
    calls++;
