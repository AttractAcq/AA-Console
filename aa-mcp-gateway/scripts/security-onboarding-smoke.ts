...
const client = new Client({ name: "aa-security-gate15", version: "1.0.0" });
let calls = 0;
try {
  ...
  const bad = await fetch(url, {
    ...
    headers: {
      authorization: "Bearer gate15-invalid-test-credential",
  ...
  await client.connect(
    new StdioClientTransport({
      command: securityDevops.connector.command,
  ...
  assertSecurityDiscovery(names);
  await runSecurityGate(async (name, args) => {
    calls++;
