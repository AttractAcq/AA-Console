/** Gate 10 uses the existing Harbour bridge, without changing any connector. */
import assert from "node:assert/strict";
import { readFileSync, statSync } from "node:fs";
import { Client } from "@modelcontextprotocol/sdk/client/index.js";
import { StdioClientTransport } from "@modelcontextprotocol/sdk/client/stdio.js";
import { distributionManager as config } from "../src/onboarding/distribution-manager.js";
import { assertDistributionDiscovery, runDistributionGate } from "./distribution-gate.js";
const client = new Client({ name: "aa-distribution-gate10", version: "1.0.0" });
let calls = 0;
try {
  ...
  assertDistributionDiscovery(names);
  await runDistributionGate(async (name, args) => {
    calls++;
    return (await client.callTool({ name, arguments: args })).structuredContent;
  }, fixtures);
  console.log(JSON.stringify({ bot_id: config.bot_id, gate: 10, calls, discovery: names.length,
    status: "PASS", at: new Date().toISOString() }));
} catch {
  console.error(JSON.stringify({ bot_id: config.bot_id, gate: 10, calls, status: "FAIL",
