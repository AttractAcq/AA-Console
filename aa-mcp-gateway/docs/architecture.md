...
Content is the first integration domain. `ContentService` delegates Production Manager tools to fixed, server-configured AA HTTP routes (`generate-brief` plus Phase 5 list/get/status/revision/approval-request/repurpose). The adapter is enabled by `AA_INTERNAL_API_URL` plus `AA_MCP_SERVICE_SECRET`. AA owns resource authorization, client grants, approved-state enforcement, transactional deduplication, and queueing. The gateway does not access Supabase or fabricate briefs. `content.approve_asset` and `content.queue_distribution` stay stubs. Local workflow approval querying is real control-plane functionality; general delivery tasks remain stubbed. Unconnected capabilities stay in the registry as stubs and are omitted from default discovery; with `MCP_DISCOVER_STUBS=true` they still return `not_implemented` with a dependency. No finance payment tool is exposed in production; a test-only critical tool proves policy enforcement.
