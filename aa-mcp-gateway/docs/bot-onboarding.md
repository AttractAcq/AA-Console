...
Marketing Gate 16 extends Gate 9: post-#48 exact **40**-tool discovery (19 reads / 21 writes) via additive grants (Gate 9's 23 plus 17 Phase 16 names). This is not the final 45. Conversion Page Builder tools are real against `client_pages` (no publish). Campaign list/get/status/writes bind to Execution OS `client_campaigns`; `ad_campaign_id` remains the attribution tracker fixture. Use `npm run smoke:marketing -- fixtures.json <private-header-file> <staging-or-local-mcp-url>`. See [Phase 16](phase-16-conversion-campaign.md); Gate 16 is NOT YET CLOSED.
