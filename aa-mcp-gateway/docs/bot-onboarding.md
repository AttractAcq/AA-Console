...
Finance (Phase 13) follows `src/onboarding/finance-controller.ts`: exact 14-tool discovery, `economics.*` wildcard converted to five named reads plus `attribution.get_revenue_attribution` and the eight-tool workflow suite. Use `npm run smoke:finance -- fixtures.json <private-header-file> <staging-or-local-mcp-url>` after the separately approved staging setup. The script refuses the known Production MCP hostname. No bank/Stripe/Xero/payments, no token mint, no money writes. See [Phase 13](phase-13-finance-controller.md); Gate 13 is NOT YET CLOSED.

Engineering (Phase 14) follows `src/onboarding/engineering-ops.ts`: exact 12-tool discovery, no wildcard grants, AA-native issue tracking plus safe page/job status projections. Use `npm run smoke:engineering -- fixtures.json <private-header-file> <staging-or-local-mcp-url>` after the separately approved staging setup. The script refuses the known Production MCP hostname. No Railway write, secret rotation, unrestricted deploy or live connector changes. See [Phase 14](phase-14-engineering-ops.md); Gate 14 is NOT YET CLOSED.

Security / DevOps (Phase 15) follows `src/onboarding/security-devops.ts`: exact 14-tool discovery, `security.*` wildcard converted to four named tools plus two engineering status reads and the eight-tool workflow suite. Use `npm run smoke:security -- fixtures.json <private-header-file> <staging-or-local-mcp-url>` after the separately approved staging setup. The script refuses the known Production MCP hostname. No secret exfiltration, unattended destroy, global clients, Railway write, secret rotation or live connector changes. See [Phase 15](phase-15-security-devops.md); Gate 15 is NOT YET CLOSED.

Marketing Gate 16c extends Gate 16: exact **45**-tool discovery (22 reads / 23 writes) after #48 then this PR. Conversion Page Builder and campaign execution stay real. Funnel/content-performance reads, `brand.get_profile`, and scoped `sites.provision` / `sites.publish_page` (HIGH+approval) are Marketing/Sales Ops owned, not Engineering. Use `npm run smoke:marketing -- fixtures.json <private-header-file> <staging-or-local-mcp-url>`. See [Phase 16](phase-16-conversion-campaign.md) and [Phase 16c](phase-16c-attribution-brand-sites.md); Gate 16c is NOT YET CLOSED.
