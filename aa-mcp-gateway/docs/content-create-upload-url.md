...
5. `content.submit_asset` with `brief_id` plus `storage_path` or `asset_id` (the returned `pending_asset_id`; `asset_id` is the same reservation id), and `media_type: "image"`. That call stays metadata-only: it inserts `client_media_assets` (`review_status=pending`) only after the object is in the bucket, then consumes the reservation.
