...
2. The gateway calls Console `/internal/mcp/content/create-upload-url`. The RPC checks the bot, the client grant, that the brief belongs to that client, and that the brief status is `draft`, `approved`, or `in_production` and not archived. It reserves `{client_id}/{pending_asset_id}.{ext}` in `client-media` for 30 minutes.
...
5. `content.submit_asset` with `brief_id` plus `storage_path` or `pending_asset_id`, and `media_type: "image"`. That call stays metadata-only: it inserts `client_media_assets` (`review_status=pending`) only after the object is in the bucket, then consumes the reservation.
...
Client `e4b4b001-81f6-4997-8429-ff21f4ee1fbe` (`clients.id`). Campaign `457e0ca8-8af6-4ead-a39d-d5326c729882` is `client_ideas.campaign_id`; briefs hang off `client_briefs.client_id` and `client_briefs.source_idea_id` (there is no `client_briefs.campaign_id`). Draft briefs on that client are accepted. The tool does not hard-code these UUIDs: the bot passes `client_id`, and the RPC requires `client_briefs.client_id` to match that grant.
